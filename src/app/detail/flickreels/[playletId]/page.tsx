...
export default function FlickReelsDetailPage() {
  ...
  const { data, isLoading, error } = useFlickReelsDetail(playletId || "");
    ...
    <main className="min-h-screen pt-20">
      <div className="relative">
        <div className="absolute inset-0 overflow-hidden">
          <img src={optimizeBg(data.cover)} alt="" className="w-full h-full object-cover opacity-20 blur-3xl scale-110" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
            <div className="relative group">
              <img src={optimizePoster(data.cover)} alt={data.title} className="w-full max-w-[300px] mx-auto rounded-2xl shadow-2xl" />
              <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-background/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6">
                <button onClick={handleWatch} className="px-8 py-3 rounded-full bg-primary text-primary-foreground font-semibold flex items-center gap-2 hover:scale-105 transition-transform shadow-lg">
            ...
            <div className="space-y-6">
              <div>
                <h1 className="text-3xl md:text-4xl font-bold font-display gradient-text mb-4">{data.title}</h1>
                ...
                {data.labels && data.labels.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {data.labels.map((label: string, idx: number) => (
                      <span key={idx} className="tag-pill">{label}</span>
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="glass rounded-xl p-4">
                ...
                <p className="text-muted-foreground leading-relaxed">{data.description || "Tidak ada deskripsi."}</p>
              </div>

              <button onClick={handleWatch} className="inline-flex items-center gap-2 px-8 py-3 rounded-full font-semibold text-primary-foreground transition-all hover:scale-105 shadow-lg" style={{ background: "var(--gradient-primary)" }}>
                <Play className="w-5 h-5 fill-current" />
                Mulai Menonton
              </button>
            </div>
